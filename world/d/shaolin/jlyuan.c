...
//	ob->move("/d/shaolin/jianyu");
	//减除少林寺的霸道！
	if ((int)ob->query("max_kee")>=1000)
	{	
		ob->move("/d/shaolin/guangchang3");
                ob->set("kee",(int)ob->query("max_kee")/5);
                ob->set("sen",(int)ob->query("max_sen")/4);
                ob->set("gin",(int)ob->query("max_gin")/4);  
		ob->set("eff_kee",(int)ob->query("max_kee")/5);
		ob->set("eff_sen",(int)ob->query("max_sen")/4);
		ob->set("eff_gin",(int)ob->query("max_gin")/4);
             message_vision( "僧兵们一涌而上，乱棒齐下，$N痛得几乎昏了过去\n",NOR,ob);
             message_vision( "还是找机会溜吧....\n",NOR,ob);
//		ob->move("/d/shaolin/guangchang3");
	}
	else
	{
