...
//write(sprintf("name %s max %d  sen_cost %d + sen_cost %d x (difficulty %d - 悟性%d ) / 20", skill["name"], skill["max_skill"],skill["sen_cost"],skill["sen_cost"],skill["difficulty"],(int)me->query("int")));
	if (cost < 0)
		cost = 1;
