// Room: /u/dunno/cave/cave24.c
// /u/dan/room/cave/cave24.c
// dan
inherit ROOM;
...
int do_studywall()
{
	...
	message_vision("$N专心地研究墙壁上的图形。\n", me); 
        ...
        if ((int)me->query_skill("literate", 1) <= 0)
         ...
         if ((int)me->query_skill("force", 1) <= 199
        ...
        if ((int)me->query_skill("force", 1) <= 299
         && (int)me->query_skill("force", 1) >= 200 
         && (int)me->query_skill("literate", 1) <= 0
               )       
        	{       if( pow(to_float((int)me->query_skill("force", 1)), 3.0) / 10.0 >to_float(me->query("combat_exp")) )
