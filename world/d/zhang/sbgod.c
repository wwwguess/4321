...
/*	if ((int)me->query("combat_exp") >=5000000){
	    message_vision("朱红雪客气地对$N,说: 
		现在还没任务，以后再说吧！\n",me);
	    return 0;		
	}
*/
	if ((int)me->query("combat_exp") <= 10){
...
/*        if (num>0)
        {
                ...
                if ((num<sizeof(levels)-1)&&(random(100)>92))
                {
                        num=num+1;
                        factor=15;
                }
...
*/
        ...
        tag=levels[num];
 if (random(10)>3) {       quest = QUEST_D(tag)->query_quest();}
...
int accept_object(object who, object ob)
{
	object pre_ob;
	 ...
	 {   who->set("quest", 0 );
             	...
             	tell_object(who,"朱鸿雪说道：好吧，这个任务就算了！");
		return 1;
	}
        ...
        { 
//	pre_ob = ob->previous_object();
       	if ((userp(ob))||(ob->query("npc")==1))
          	{
                 tell_object(who,HIY "朱红雪说：这不是我想要的。\n" NOR);
         	    return 0;
        	}
        //destruct(ob);
