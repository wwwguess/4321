// Room: /u/dunno/cave/cave18.c
// /u/dan/room/cave/cave18.c
// dan
inherit ROOM;
...
LONG
	...
	set("exits", ([ /* sizeof() == 2 */
  "north" : __DIR__"cave17",
  "south" : __DIR__"cave19",
]));
...
int do_studywall()
{
	...
	message_vision("$N专心地研究墙壁上的图形。\n", me); 
        ...
        if ((int)me->query_skill("literate", 1) <= 0)
         ...
         if ((int)me->query_skill("stuff", 1) <= 99
         && (int)me->query_skill("literate", 1) <= 0
               ) 
                {
                        tell_object(me, "这些对于你来说太深了！！！\n");
                 }
        if ((int)me->query_skill("stuff", 1) >= 200
         && (int)me->query_skill("literate", 1) <=0
               ) 
                {
                        tell_object(me, "这些对于你来说太浅了！！！\n");
                 }
        if ((int)me->query_skill("stuff", 1) <= 199
         && (int)me->query_skill("stuff", 1) >= 100 
         && (int)me->query_skill("literate", 1) <= 299 
               )       
        	{       if( pow(to_float((int)me->query_skill("stuff", 1)), 3.0) / 10.0 >to_float(me->query("combat_exp")) )
                        return notify_fail("你经验不足，快去练功.\n");
	        	me->receive_damage("gin", 20);
	        	me->improve_skill("stuff", (me->query("int")));
		        tell_object(me, "你的基本棍法有所提高！！！\n");
	        }
