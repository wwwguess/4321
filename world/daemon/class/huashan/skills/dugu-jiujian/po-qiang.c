...
int perform(object me, object target)
{
		...
		&& (string)weapon->query("skill_type") != "qiang" 
		&& (string)weapon->query("skill_type") != "ji"))
                return notify_fail("你的对手没有用长兵器，不能使用「破枪式」。\n");
        if( (int)me->query_skill("dugu-jiujian", 1) < 50 )
                return notify_fail("你的独孤九剑不够娴熟，不会使用「破枪式」。\n");
        if( (int)me->query_temp("po") ) 
		return notify_fail("你上一招还没使完，不能再使「破枪式」。\n");

        skill = me->query_skill("sword")+me->query_skill("dugu-jiujian");

        msg = CYN "$N使出独孤九剑「破枪式」，把剑舞成一片，从一个怪异的角度直刺$n招中破绽。\n";
