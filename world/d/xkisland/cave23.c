// Room: /u/dunno/cave/cave23.c
// /u/dan/room/cave/cave23.c
// dan
inherit ROOM;
...
LONG
	...
	set("exits", ([ /* sizeof() == 2 */
  "north" : __DIR__"cave22",
  "south" : __DIR__"cave24",
]));
...
int do_studywall()
{
	...
	message_vision("$N专心地研究墙壁上的图形。\n", me); 
        ...
        if ((int)me->query_skill("literate", 1) <= 0)
         ...
         if ((int)me->query_skill("finger", 1) <= 99
        ...
        if ((int)me->query_skill("finger", 1) <= 199
         && (int)me->query_skill("finger", 1) >= 100 
         && (int)me->query_skill("literate", 1) <= 0
               )       
        	{       if( pow(to_float((int)me->query_skill("finger", 1)), 3.0) / 10.0 >to_float(me->query("combat_exp")) )
                        return notify_fail("你经验不足，快去练功.\n");
	        	me->receive_damage("gin", 20);
	        	me->improve_skill("finger", (me->query("int")));
		        tell_object(me, "你的基本指法有所提高！！！\n");
	        }
