...
LONG
  ...
  "down"  : __DIR__"quicksand5",
        ...
        setup();
        san = present("skeleton", this_object());
        sna = present("buddha-tooth", this_object());
        sna->move(san);
        sna = present("snake", this_object());
        sna->move(san);
        sna = present("snake", this_object());
        sna->move(san);
...
{       int i;
        ...
        {
                if( random(i/3+10)< 1)                drop_him(ob);
        }
        else
        {
                if( random(i+10)< 2)      drop_him(ob);
        }
        }
