...
varargs int skill_power(object ob, string skill, int usage)
{
		...
		write(sprintf(("skill %s level %d %d\n"), skill, level,usage));
...
// Are we in a peace room?
        ...
        if(  (me->query_cps())*50 + (int)me->query("force") > (random(bellicosity) + 
...
// add in May 27, 1996 (1) -- 3 line
	...
	if ( !killer->is_living()  && objectp(killer->query("possessed"))) 
       ...
       if (victim->query_temp("nodie")!=1){
 if (victim->query("age")>=15)   {
